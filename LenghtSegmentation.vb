...
Public Class LengthSegmentation
    ...
    Private _JunctionCells() As Cells
    ...
    Public Sub IdentifyJunctionsAndOutlet()
        ...
        _JunctionCells(_NumberOfJunctionCells) = New Cells(_yOutlet, _xOutlet)
    ...
    ''' </summary>
    ''' <param name="minLength">Descrição: o comprimento mínimo para a rede de drenagem</param>
    Public Sub SegmentBySpecificLength(ByVal minLength As Single)
        ...
        For nJunction = _NumberOfJunctionCells To 0 Step -1 'Faz da maior área para a menor
            ...
            Do 'Faça até chegar na cabeceira do rio mais longo
                ...
                If row = yAnterior AndAlso col = xAnterior Then 'Significa que chegou na célula de cabeceira
                    ...
                    Do While totalLength < minLength 'Faça até atingir o limite de extensão
                ...
                totalLength += (SpatialAnalyst.CellLength(yLat, xLon, (yAnterior - row), (xAnterior - col), _FlowDirection.Cellsize)) * 1000 'Incrementa o comprimento acumulado
                If totalLength > minLength Then 'Se comprimento acumulado for maior do que o limiar estabelecido
                            ...
                            If _StrSeg.Dados(yi, xi) > 0 Then 'Somente se a célula faz parte da rede de drenagem principal
                                If FlowDirection.RelativeIncipientFlowDirection(yi, Yc, xi, xc) = _FlowDirection.Dados(yi, xi) Then
                                    ...
                                    If _CellAnalyzed(yi, xi) = False Then 'Adiciona a célula somente se ela não tenha sido adicionada ainda
                                        ...
                                        _JunctionCells(_NumberOfJunctionCells) = New Cells(yi, xi) 'Adiciona o exutório do contribuinte
