            ...
            Do 'Faça até chegar na cabeceira do rio mais longo
                ...
                If row = yAnterior AndAlso col = xAnterior Then 'Significa que chegou na célula de cabeceira
                    ...
                    Do While totalLength < _minLength 'Faça até atingir o limite de extensão
                        ...
                        totalLength += (SpatialAnalyst.CellLength(yLat, xLon, (yAnterior - row), (xAnterior - col), _FlowDirection.Cellsize)) * 1000 'Incrementa o comprimento acumulado
...
#Region "Segment 'Equal Catchment Lengths - style'"
...
restartPosition:
    ...
    Public Sub DoubleSegmentedPosProcessing()

        ReDim _CellAnalyzed(_StrSeg.Linhas, _StrSeg.Colunas) 'Reinicializa o vetor das células analizadas
        Dim xAnterior, yAnterior As Int16
        Dim row, col As Int16
        Dim MovedUpJunction As Boolean
        Dim SegmentCode, PreviousSegmentCode As Integer

        For nJunction = (_NumberOfJunctionCells - 1) To 0 Step -1 'Faz da maior área para a menor, ignorando o exutório final
            ...
            If _StrSeg.Dados(row, col) = _StrSeg.NoDataValue Then GoTo nextjunction 'Se a célula já foi analisada e removida da drenagem principal, passa para a próxima junção
            FlowDirection.MoveToFlowDirection(_FlowDirection.Dados(row, col), row, col) 'Anda uma célula para jusante para alcançar a célula de junção (com exceção do exutório da bacia '0')

            SegmentCode = _StrSeg.Dados(row, col) 'Identifica o número do segmento atual

            If SegmentCode = _StrSeg.NoDataValue Then 'Significa que o trecho a jusante já foi deletado. Deverá ser deletado o trecho atual
                row = yAnterior : col = xAnterior 'Volta para a posição anterior
                SegmentCode = _StrSeg.Dados(yAnterior, xAnterior)
            ...
            If MovedUpJunction = True Then 'Caso passar da junção e ir em direção à menor área, começa a exclusão do trecho

                Do 'Faça até chegar em uma junção ou trocar de código de segmentação

                    SegmentCode = _StrSeg.Dados(row, col) 'Identifica o código do segmento após a mudança de posição

                    If yAnterior = row AndAlso xAnterior = col Then Exit Do 'Chegou na célula de cabeceira
EraseCells:
                    If PreviousSegmentCode <> SegmentCode Then Exit Do 'Se trocar o código do segmento, vai para a próxima junção
                    _StrSeg.Dados(row, col) = _StrSeg.NoDataValue 'Apaga a definição de rede de drenagem da célula
                    ...
                    If MovedUpJunction = True Then 'Chegou em uma nova junção, sai do loop
...
nextjunction:
    ...
    'Move célula rio acima
    Private Sub MoveToUpstreamMinArea(ByVal Yc As Int16, ByVal Xc As Int16, ByRef rowUpstr As Int16, ByRef colUpstr As Int16, ByRef AfterJunction As Boolean, ByVal markOutlets As Boolean)

        Dim xi, yi As Int16
        Dim MinUpstreamArea As Single = Single.MaxValue 'Em direção à menor área de drenagem
        Dim CounterPointingCells As Int16 = 0 'Conta quantas células apontaram para a célula atual
            ...
            For x = -1 To 1
                ...
                If x <> 0 OrElse y <> 0 Then 'Evita a análise da própria célula (nó central)
                    If xi >= 0 AndAlso yi >= 0 AndAlso xi < _FlowDirection.Colunas AndAlso yi < _FlowDirection.Linhas Then 'Evita sair dos limites do raster
                        ...
                        If _StrSeg.Dados(yi, xi) > 0 Then 'Se estiver em cima da rede de drenagem

                            If FlowDirection.RelativeIncipientFlowDirection(yi, Yc, xi, Xc) = _FlowDirection.Dados(yi, xi) Then

                                CounterPointingCells += 1
                                If markOutlets = True Then _CellAnalyzed(yi, xi) = True 'Marca o exutório para a respectiva drenagem ser removida posteriormente

                                'Verifica qual a célula com a menor área contribuinte
                                If _FlowAcc.Dados(yi, xi) < MinUpstreamArea Then
                                    MinUpstreamArea = _FlowAcc.Dados(yi, xi)
                                    rowUpstr = yi
                                    colUpstr = xi
                                End If
                            End If
        ...
        If CounterPointingCells > 1 Then AfterJunction = True 'Avisa que chegou em uma junção e moveu para a célula de menor área
